...
void SimplePage(BufferPoolManager *bpm, int i) {
  page_id_t page_id;
  auto *page = bpm->NewPage(&page_id);
  ASSERT_NE(nullptr, page);
  ASSERT_EQ(1, page->GetPinCount());
  ASSERT_EQ(page_id, page->GetPageId());
  snprintf(page->GetData(), PAGE_SIZE, "%d", i);

  EXPECT_TRUE(bpm->UnpinPage(page_id, true));
  ASSERT_EQ(0, page->GetPinCount());
...
TEST(BufferPoolManagerTest, SimplePageTest) {
  ...
  SimplePage(bpm, 0);
  EXPECT_EQ(1, disk_manager->GetNumWrites());
